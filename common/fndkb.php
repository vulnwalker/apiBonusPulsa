...
class DkbObj extends DaftarObj2{
	...
	function genDaftarOpsi($tipe=''){
		...
		//
		if($jns=='windowshow'){
		$bdg=sqlArray(sqlQuery("SELECT * FROM ref_skpd where c='".$_REQUEST[$this->Prefix.'SkpdfmSKPD']."' and d='00'"));
		$bidang = $bdg['c'].'. '.$bdg['nm_skpd'];
		$unt=sqlArray(sqlQuery("SELECT * FROM ref_skpd where c='".$_REQUEST[$this->Prefix.'SkpdfmSKPD']."' and d='".$_REQUEST[$this->Prefix.'SkpdfmUNIT']."' and e='00'"));
	...
	function simpanDKBMD(){
	 ...
	 //ambil thn_akun
	// $row=sqlArray(sqlQuery("select * from ref_jurnal where ka='$k' and kb='$l' and kc='$m' and kd='$n' and ke='$o' and kf='$kf'"));
