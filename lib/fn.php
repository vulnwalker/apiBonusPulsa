...
Function UserLogin() { 
	...
	$Cek1 = sqlQuery("select * from admin where uid='$User' and password = md5('$Pwd')"); 
	if(sqlNumRow($Cek1)) { 
		$isi = sqlArray($Cek1); 
		$Nama = $isi['nama']; 
		$UserId = $isi['uid']; 
	...
	if($Status) { 
		...
		$OnLine = sqlQuery("update admin set online='1' where uid='$User'"); 
		return true; 
	} else { 
		return false; 
	} 
...
function JmlOnLine() { 
	$n = 0; 
	$n = sqlNumRow(sqlQuery("select * from admin where online='1'")); 
	return $n; 
} 
...
function PrintSKPD() { global $fmWIL,$fmSKPD,$fmUNIT,$fmSUBUNIT,$fmTAHUNANGGARAN,$fmKEPEMILIKAN,$Main,$HTTP_COOKIE_VARS; $KEPEMILIKAN = sqlArray(sqlQuery("select nm_pemilik from ref_pemilik where a1='$fmKEPEMILIKAN'"));$KEPEMILIKAN = $KEPEMILIKAN[0]; $WILAYAH = sqlArray(sqlQuery("select nm_wilayah from ref_wilayah where b='$fmWIL' "));$WILAYAH = $WILAYAH[0]; $SKPD = sqlArray(sqlQuery("select nm_skpd from ref_skpd where d='00' and c='$fmSKPD' "));$SKPD = $SKPD[0]; $UNIT = sqlArray(sqlQuery("select nm_skpd from ref_skpd where c='$fmSKPD' and d <> '00' and e = '00' and d='$fmUNIT' "));$UNIT = $UNIT[0]; $SUBUNIT = sqlArray(sqlQuery("select nm_skpd from ref_skpd where c='$fmSKPD' and d = '$fmUNIT' and e <> '00' and e='$fmSUBUNIT' "));$SUBUNIT = $SUBUNIT[0]; $Hsl ="
Provinsi	:	JAWA BARAT
Kabupaten/Kota	:	$WILAYAH
SKPD	:	$SKPD
UNIT	:	$UNIT
SUB UNIT	:	$SUBUNIT
"; return $Hsl; } function PrintTTD() { global $fmWIL,$fmSKPD,$fmUNIT,$fmSUBUNIT,$fmTAHUNANGGARAN,$fmKEPEMILIKAN,$Main,$HTTP_COOKIE_VARS; $NIPSKPD = ""; $NAMASKPD = ""; $JABATANSKPD = ""; $TITIMANGSA = "Bandung, ".JuyTgl1(date("Y-m-d")); if (c=='04') { $Qry = sqlQuery("select * from ref_pejabat where c = '$fmSKPD' and d = '$fmUNIT' and e = '$fmSUBUNIT' and ttd1 = '1' "); } else { $Qry = sqlQuery("select * from ref_pejabat where c = '$fmSKPD' and d = '$fmUNIT' and e = '00' and ttd1 = '1' "); } while($isi=sqlArray($Qry)) { $NIPSKPD1 = $isi['nik']; $NAMASKPD1 = $isi['nm_pejabat']; $JABATANSKPD1 = $isi['jabatan']; } $Qry = sqlQuery("select * from ref_pejabat where c = '$fmSKPD' and d = '$fmUNIT' and e = '$fmSUBUNIT' and ttd2 = '1' "); while($isi=sqlArray($Qry)) { $NIPSKPD2 = $isi['nik']; $NAMASKPD2 = $isi['nm_pejabat']; $JABATANSKPD2 = $isi['jabatan']; } $Hsl = "
 	 MENGETAHUI
KEPALA SKPD,
