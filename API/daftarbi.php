...
$limit=10;
	...
	$jml_data = sqlArray(sqlQuery($qry_jml));
 ...
 $response["jml_data"] = $jml_data['cnt'];
	if (sqlNumRow($hasil) > 0) {
		...
		//ambil gambar di gambar()
			...
			$qry_gbr=sqlQuery($aqry_gbr);
			$h['gambar']=array();
			while ($arrgambar=sqlArray($qry_gbr)){
