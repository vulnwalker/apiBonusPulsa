...
$jmlTotalHargaDisplay = sqlQuery("select sum(pengadaan.jml_harga) as total  from pengadaan inner join dkb using(f,g,h,i,j) inner join ref_barang using(f,g,h,i,j) where $Kondisi ");
...
$Qry = sqlQuery("select pengadaan.*,ref_barang.nm_barang from pengadaan inner join ref_barang using(f,g,h,i,j) where $Kondisi order by a,b,c,d,e,f,g,h,i,j $LimitHal");
