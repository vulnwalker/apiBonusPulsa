...
if($Act=="Simpan")
{
	if(ProsesCekField($MyField))
		...
		if($Baru=="1")
		{
			...
			//echo $Qry;
			$Simpan = sqlQuery($Qry);
			$UpdateBI = sqlQuery("update buku_induk set status_barang='5' where id='$fmIDBUKUINDUK'");
			...
			$Q = sqlArray(sqlQuery("select id from gantirugi where concat(a1,a,b,c,d,e,f,g,h,i,j,id_bukuinduk,noreg,thn_perolehan)='$KondisiCek' order by id desc"));
			$fmID = $Q['id'];
			echo $fmID;
			*/
		}
...
if($Act=="Edit"|| $Act == "TambahEdit")
{
	...
	else
	{
		if($Act=="Edit")
		{
			$Qry = sqlQuery("select * from gantirugi where id='{$cidNya[0]}'");
		}
		else
		{
...
if($Act=="Hapus" && count($cidTGR) > 0)
{
	...
	{
		$cekIdBI = sqlArray(sqlQuery("select id_bukuinduk from gantirugi where id='{$cidTGR[$i]}'"));$cekIdBI = $cekIdBI[0];
		$Del = sqlQuery("delete from gantirugi where id='{$cidTGR[$i]}' limit 1");
...
$Qry = sqlQuery("select gantirugi.*,ref_barang.nm_barang from gantirugi inner join ref_barang using(f,g,h,i,j) where $Kondisi order by a1,a,b,c,d,e,f,g,h,i,j,noreg ");
$jmlDataTGR = sqlNumRow($Qry);
$Qry = sqlQuery("select gantirugi.*,ref_barang.nm_barang from gantirugi inner join ref_barang using(f,g,h,i,j) where $Kondisi order by a1,a,b,c,d,e,f,g,h,i,j,noreg $LimitHalTGR");
