...
class pemasukanObj  extends DaftarObj2{	
   ...
   function setBuatLaporan(){
	...
	$qrypenerimaan_det = "SELECT * FROM ".$DataPengaturan->VPenerima_det()." WHERE refid_terima='".$isi['Id']."' AND sttemp='0'";
...
//					"<input type='hidden' name='pemasukanSKPDfmUrusan' value='".$_REQUEST['pemasukanSKPDfmUrusan']."' />".
	...
	function Hapus($ids){ //validasi hapus ref_pegawai
		...
		if($err == ''){
			for($i = 0; $i<count($ids); $i++){
				...
				unlink("Media/PENGADAAN/".$ids[$i].".png");
	...
	function ProsesPosting(){
		...
		if(intval($thn_anggaran) < intval(date('Y')))$tgl_buku = $thn_anggaran."-12-31"; 	
		...
		$harga_perolehan_satuan = round($harga_perolehan_satuan,2) ;
		...
		if($harga_baru > $harga_perolehan){
			...
			$harga_perolehan_satuan_akhir = round(($harga_perolehan_satuan-$harga_perolehan_satuan_akhir));
			...
			$harga_perolehan_satuan_akhir = round($harga_perolehan_satuan) * ($daqry_pen_det['jml']-1);
		...
		$harga_beli_barang = round($harga_beli_barang,2);
		...
		$harga_atribusi_barang = round($harga_atribusi_barang,2);	
	...
	function HapusPosting(){
		...
		if($err == ''){
			$qry_del = "DELETE FROM buku_induk WHERE refid_terima = '$idplh' LIMIT 100";$cek.=$qry_del;
