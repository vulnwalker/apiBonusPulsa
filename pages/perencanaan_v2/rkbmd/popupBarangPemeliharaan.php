	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		$kode_barang=$get['f'].'.'.$get['g'].'.'.$get['h'].'.'.$get['i'].'.'.$get['j'];
		 ...
		 $getKebutuhanOptimal = sqlArray(sqlQuery("select sum(jml_barang) as kebutuhanOptimal from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and (kondisi = '1' or kondisi = '2' or kondisi = '3') "));	
		 ...
		 $getKondisiKurangBaik = sqlArray(sqlQuery("select sum(jml_barang) as kurangBaik from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '2'" ));		
		 $kurangBaik = $getKondisiKurangBaik['kurangBaik'];
		 $getKondisiRusakBerat = sqlArray(sqlQuery("select sum(jml_barang) as rusakBerat from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '3'" ));		
...
/*			'' => array( 
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $getMinID = sqlArray(sqlQuery("select min(Id) as min from view_barang_rkbmd_pemeliharaan where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat'"));
