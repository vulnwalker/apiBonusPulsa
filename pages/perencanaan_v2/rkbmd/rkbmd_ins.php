...
class rkbmd_ins_v2Obj  extends DaftarObj2{	
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'subDelete':{				
			...
			sqlQuery("delete from temp_rkbmd_pengadaan_v2 where id='$id'");	
			$getRow = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));	
			if($getRow > 0){
				$content = "ada";
			}else{
				$content = "";
		...
		case 'subEdit':{				
			...
			if($jumlah > $jumlahKebutuhanRiil && $jumlahKebutuhanRiil != '0' && !empty($jumlahKebutuhanOptimal) && !empty($jumlahKebutuhanMaksimal)){
				...
				sqlQuery(VulnWalkerUpdate('temp_rkbmd_pengadaan_v2',$data,"id = '$id'"));
			}
		...
		case 'finish':{				
			$username = $_COOKIE['coID'];
			$execute = sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user='$username'");	
			...
			sqlQuery("delete from tabel_anggaran where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan = '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PENGADAAN') and status_validasi !='1' ");
			...
			$cekKegiatan = sqlNumRow(sqlQuery("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and ck = '$ck' and p = '$p' and q= '$q' and  f ='00' and id_tahap='$this->idTahap' and uraian_pemeliharaan = 'RKBMD PENGADAAN'"));												
			if($cekKegiatan < 1){
				$data = array('jenis_anggaran' => $this->jenisAnggaran,
							  'tahun' => $this->tahun,
							  'c1' => $c1,
			...
			$execute2 = sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user='$username'");
		...
		case 'Simpan':{
			...
			$username = $_COOKIE['coID'];
			$cekAda = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where  f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and user='$username'"));
			if($cekAda == 1){
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --'); 
		...
		case 'subShowEdit':{
			...
			$get = sqlArray(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where id = '$id'"));
			foreach ($get as $key => $value) { 
			  $$key = $value; 
			}
		    $concat2 = $f.".".$g.".".$h.".".$i.".".$j;
   ...
   function setPage_OtherScript(){
	...
	$cekMayoritas = sqlArray(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user = '$this->username'"));
	$e = $cekMayoritas['e'];
	$e1 = $cekMayoritas['e1'];
	...
	$cekGordon = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));
	if($cekGordon > 0){
		$cokot = sqlArray(sqlQuery("select * from temp_rkbmd_pengadaan_v2 where user = '$username'"));
		$selectedBK = $cokot['bk'];
		$selectedCK = $cokot['ck'];
		$selectedP = $cokot['p'];
		$selecteQ = $cokot['q'];
