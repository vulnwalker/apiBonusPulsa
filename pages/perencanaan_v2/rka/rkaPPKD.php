...
class rkaPPKD_v2Obj  extends DaftarObj2{	
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		if(!empty($this->jenisForm)){
			...
			$getIdTahapRKATerakhir = sqlArray(sqlQuery("select max(id_tahap) as max from tabel_anggaran where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_rka !=''  and (rincian_perhitungan !='' or j !='000' ) and nama_modul = 'RKA-PPKD' "));
		 	$idTahap = $getIdTahapRKATerakhir['max'];
		}

		$getData = sqlArray(sqlQuery("select sum(jumlah_harga) from tabel_anggaran where  (rincian_perhitungan !='' or j !='000' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and nama_modul='RKA-PPKD' and jenis_anggaran = '$this->jenisAnggaran' $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = sqlArray(sqlQuery("select * from tabel_anggaran where id_anggaran = '$rkaPPKD_v2_idplh'"));
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
		...
		if(!empty($this->idTahap)){
			...
			$getAllRekeningFromRka = sqlQuery("select * from tabel_anggaran where jenis_rka !='' and jenis_rka !='RKA-PPKD'  and c1 ='0' and f1='0' and k !='0' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and nama_modul ='RKA-PPKD' ");	
...
if(empty($rkaPPKD_v2->tahun)){
	...
	$getCurrenttahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD_v2->idTahap'"));
	$rkaPPKD_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD_v2->idTahap'"));
