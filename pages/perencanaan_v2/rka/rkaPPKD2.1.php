...
class rkaPPKD21_v2Obj  extends DaftarObj2{	
	...
	function genRowSum($ColStyle, $Mode, $Total){
		...
		$getData = sqlArray(sqlQuery("select sum(jumlah_harga) from tabel_anggaran where (rincian_perhitungan !='' or f !='00' ) and id_tahap='$idTahap' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and jenis_rka = '2.1' and nama_modul='RKA-PPKD' $kondisiSKPD $kondisiRekening"));
		...
		$ContentTotalHal=''; $ContentTotal='';
			...
			if($this->jenisForm == "PENYUSUNAN"){
				...
				$getRApbd = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$urutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
				if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
					$urutSebelumnya = $urutSebelumnya - 1;		
				}
				$getDataSebelumnya = sqlArray(sqlQuery("select sum(jumlah_harga) from view_rka_ppkd_2_1 where  (rincian_perhitungan !='' or f !='00' ) and no_urut='$urutSebelumnya' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'  $kondisiSKPD $kondisiRekening"));
	...
	$getJumlahBarang = sqlArray(sqlQuery("select * from tabel_anggaran where id_anggaran = '$rkaPPKD21_v2_idplh'"));
	...
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
	 ...
	 if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'setGrup':{
				...
				if(sqlNumRow(sqlQuery("select * from tabel_anggaran where o1 = '$pekerjaan' and id_tahap = '$this->idTahap' and jenis_rka = '2.1' and nama_modul ='RKA-PPKD'")) == 0){
					$data = array(
												 'c1' => '0',
												 'c' => '00',
												 'd' => '00',
		...
		case 'clearTemp':{
				$username =$_COOKIE['coID'];
				sqlQuery("delete from temp_rka_ppkd_21_v2 where user ='$username'");	
				foreach ($_REQUEST as $key => $value) { 
			 	 $$key = $value; 
				}
		...
		case 'sesuai':{
			...
			$getrkaPPKD21_v2nya = sqlArray(sqlQuery($queryRows));
		...
		case 'koreksi':{
			...
			foreach ($getrkaPPKD21_v2nya as $key => $value) { 
			 ...
			 if($this->jenisForm !='KOREKSI'){
			 	...
			 	if($err == ""){
				 	if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where  o1='$o1' and rincian_perhitungan='' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
				 	...
				 	if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where  rincian_perhitungan ='' and c1='0' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap'  ")) > 0){
		...
		case 'editTab':{
			...
			$getAll = sqlQuery("select * from view_rka_ppkd_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD'  and status_validasi !='1'  order by o1, rincian_perhitungan");
			sqlQuery("delete from temp_rka_ppkd_21_v2 where user='$username'");
			sqlQuery("delete from temp_rincian_volume_21 where user='$username'");
		    sqlQuery("delete from temp_alokasi_rka_ppkd_21_v2 where user='$username'");
			$noUrutPekerjaan = 0;
			$angkaO2 = 0;
			$lastO1 = 0;
			while($rows = sqlArray($getAll)){
				...
				$getMaxID = sqlArray(sqlQuery("select  max(id) as gblk from temp_rka_ppkd_21_v2 where user ='$username'"));
				$maxID = $getMaxID['gblk'];
				$lastO1 = $o1;
				
				$getLastO1 = sqlArray(sqlQuery("select o1 from temp_rka_ppkd_21_v2 where id='$maxID' "));
				...
				$query = VulnWalkerInsert('temp_rka_ppkd_21_v2',$data);
				sqlQuery($query);
				$angkaO2 = $angkaO2 + 1;
   ...
   function setPage_OtherScript(){
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($this->jenisForm == 'PENYUSUNAN'){
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$this->idTahap'"));
			 }else{
	 ...
	 //TAHAP PENYUSUNAN
			 ...
			 $getRApbd = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
			 if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
					$nomorBefore = $nomorBefore - 1;		
			 }
			 $getLastTahap = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			 $blackList = "";
			 if($getLastTahap['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				$getAllChild = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '".$getLastTahap['id_tahap']."' and (rincian_perhitungan !='' or j !='000'  and status_validasi !='1' )");			
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$nomorBefore' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and o1 = '$o1' and o1 !='0' and o1 !='' "));
						if($cekAdaParent == 0)$jarak = "0px";
						$Koloms[] = array('align="left"',"<span style='margin-left:$jarak;font-weight:bold;'>".$namaRekening."</span>" );
					}
				}
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $blackList"));
			 }else{
			 	$getTotalJumalhHarga = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and no_urut ='$nomorBefore' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' $blackList"));	
			 }
			 $Koloms[] = array('align="right"', " <span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')." </span>" );
			 ...
			 $getAngkaKoreksi = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where c1='$c1' and c='$c' and d='$d'  and e='$e' and e1='$e1'  and id_jenis_pemeliharaan='$id_jenis_pemeliharaan' and  f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan ='$rincian_perhitungan'  and id_tahap='$this->idTahap'"));
			 ...
			 if(empty($rincian_perhitungan)){
			    ...
			    $Koloms[] = array('align="right"',"");
				 if($c1 =='0'){
				 	if($k == '0' && $n =='0'){
					 	$getTotalJumalhHargaKoreksi = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where  o1='$o1' $kondisiSKPD and id_tahap ='$this->idTahap' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
					 }else{
	 	...
	 	if($this->jenisFormTerakhir == "KOREKSI"){
			 ...
			 $getAngkaKoreksi = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where c1='$c1' and c='$c' and d='$d'  and e='$e' and e1='$e1'  and id_jenis_pemeliharaan='$id_jenis_pemeliharaan' and  f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and o1='$o1' and rincian_perhitungan ='$rincian_perhitungan'  and no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
			 $koreksiVolumeBarang = number_format($getAngkaKoreksi['volume_rek'] ,0,',','.');
			 $koreksiSatuanHarga = number_format($getAngkaKoreksi['jumlah'] ,2,',','.');
			 $koreksiJumlahHarga = number_format($getAngkaKoreksi['volume_rek']  * $getAngkaKoreksi['jumlah'] ,2,',','.');
			 if($getAngkaKoreksi['volume_rek'] > $isi['volume_rek'] ){
			 ...
			 if(empty($rincian_perhitungan)){
			    ...
			    $Koloms[] = array('align="right"',"");
				 if($c1 =='0'){
				 	if($k == '0' && $n =='0'){
					 	$getTotalJumalhHargaKoreksi = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where  o1='$o1' $kondisiSKPD and no_urut='$this->urutTerakhir' and tahun='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'"));
					 }else{
			 ...
			 if($c1 == '0'){
			 	if(strlen($k) > 1){
					...
					if($k == '0' && $n == '0'){
						...
						$jarak = '15px';
				 		$cekAdaParent = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and o1 = '$o1' and o1 !='0' and o1 !='' "));
						if($cekAdaParent == 0)$jarak = "0px";
						$Koloms[] = array('align="left"',"<span style='margin-left:$jarak;font-weight:bold;'>".$namaRekening."</span>" );
					}
				...
				$ilustrasi = "";
				$getSumJumlahBarang = sqlArray(sqlQuery("select sum(volume_rek) as total from view_rka_ppkd_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
				$Koloms[] = array('align="left"',"" );
				$jumlahBarang = $getSumJumlahBarang['total'];
				//$Koloms[] = array('align="right"', number_format($jumlahBarang ,0,',','.') );
			 ...
			 $Koloms[] = array('align="left"', $satuan_rek );
			 $getSumSatuanRek = sqlArray(sqlQuery("select sum(jumlah) as total from view_rka_ppkd_2_1 where k='$k' and l='$l' and m='$m' and n='$n'  and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));
			 $sumSatuanRek = $getSumSatuanRek['total'];
			 if($c1 == '0'){
			 	//$Koloms[] = array('align="right"', number_format($sumSatuanRek ,2,',','.') );
				$Koloms[] = array('align="right"','' );
			 ...
			 if($c1 == '0'){
			 if($k == '0' && $n =='0'){
			 	$getTotalJumalhHarga = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where o1='$o1' $kondisiSKPD $kondisiBelanja and id_tahap='$idTahap'"));
			 }else{
			 	$getTotalJumalhHarga = sqlArray(sqlQuery("select sum(jumlah_harga) as total from view_rka_ppkd_2_1 where k='$k' and l='$l' and m='$m' and n='$n' and o='$o' $kondisiSKPD and id_tahap='$idTahap'"));	
			 }
			 $Koloms[] = array('align="right"', "<span style='font-weight:bold;'>".number_format($getTotalJumalhHarga['total'] ,2,',','.')."</span>" );
			 if($this->wajibValidasi == TRUE)$Koloms[] = array('align="center"', "");
			 }else{
...
$fmORDER1 = $_REQUEST['fmORDER1'];
	...
	function getDaftarOpsi($Mode=1){
	  ...
	  if(isset($cmbUrusan) && $cmbUrusan== ''){
			...
			$arrKondisi[] = "c1 != '0'";
		if($this->jenisForm == 'PENYUSUNAN'){
			$getAllParent = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");	
			while($rows = sqlArray($getAllParent)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
				}
				$cekPekerjaan = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap='$this->idTahap'  or id_anggaran = '$id_anggaran' ";
					$getAllRekening = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$this->idTahap'  and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");				
					while($row2s = sqlArray($getAllRekening)){
						foreach ($row2s as $key => $value) { 
					 	 $$key = $value; 
						}
						$cekRekening = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$this->idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
				...
				$grabNonMapingRekening= sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = sqlArray($grabNonMapingRekening)){
					if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!=''  $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$this->idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
					
				}
				$grabNonHostedRekening= sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$this->idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
			...
			$nomorUrutSebelumnya = $this->nomorUrut - 1;
			$getRApbd = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' "));
			if($getRApbd['jenis_form_modul'] != "PENYUSUNAN" && $getRApbd['jenis_form_modul'] != "KOREKSI"){
				$nomorUrutSebelumnya = $nomorUrutSebelumnya - 1;		
			}
			$getLastTahap = sqlArray(sqlQuery("select * from view_rka_ppkd_2_1 where no_urut = '$nomorUrutSebelumnya' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'"));
			$blackList = "";
			if($getLastTahap['jenis_form_modul'] == "PENYUSUNAN" && $this->wajibValidasi == TRUE){
				$getAllChild = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '".$getLastTahap['id_tahap']."' and (rincian_perhitungan !='' or j !='000'  and status_validasi !='1' )");			
				while($black = sqlArray($getAllChild)){
			...
			$idTahap = $getIDTahap['id_tahap'];
			$getAllParent = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap'  and o1 !='0' and k='0' and n='0' and rincian_perhitungan=''  ");	
			while($rows = sqlArray($getAllParent)){
				foreach ($rows as $key => $value) { 
			 	 $$key = $value; 
				}
				$cekPekerjaan = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
				if($cekPekerjaan == 0){
					$arrKondisi[] = "o1 !='$o1'";
				}else{
					$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran' ";
					$getAllRekening = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and c1 = '0' and j ='000' and rincian_perhitungan = '' and k!='0' and n !='0'  ");				
					while($row2s = sqlArray($getAllRekening)){
						foreach ($row2s as $key => $value) { 
					 	 $$key = $value; 
						}
						$cekRekening = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
						if($cekRekening == 0){
							$concat = $k.".".$l.".".$m.".".$n.".".$o;
							$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
						}else{
							$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran'  ";
							$getAllProgram = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and p != '0' and q ='0' and j ='000' and rincian_perhitungan = ''   ");				
							while($row3s = sqlArray($getAllProgram)){
								foreach ($row3s as $key => $value) { 
							 	 $$key = $value; 
								}
								$cekProgram = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
								if($cekProgram == 0){
									$concat = $bk.".".$ck.".".$p;
									$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
								}else{
									$arrKondisi[] = " id_tahap = '$idTahap'  or id_anggaran = '$id_anggaran'   ";
									$getAllKegiatan = sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and bk ='$bk' and ck='$ck' and p='$p' and q != '0'  and j ='000' and rincian_perhitungan = ''   ");				
									while($row4s = sqlArray($getAllKegiatan)){
										foreach ($row4s as $key => $value) { 
									 	 $$key = $value; 
										}
										$cekKegiatan = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap = '$idTahap' $kondisiRekening $kondisiSKPD  $blackList and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
										if($cekKegiatan == 0){
											$concat = $bk.".".$ck.".".$p;
											$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
										}else{
				...
				$grabNonMapingRekening= sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = sqlArray($grabNonMapingRekening)){
					if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
					
				}
				
				$grabNonHostedRekening= sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and (LENGTH(k) = 1 and k !='0') and rincian_perhitungan=''");
				while($got = sqlArray($grabNonHostedRekening)){
					if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap = '$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD $blackList")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
			...
			if($this->jenisFormTerakhir == "KOREKSI"){
				...
				while($rows = sqlArray($getAllParent)){
					...
					$cekPekerjaan = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where o1 = '$o1' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
					if($cekPekerjaan == 0){
						$arrKondisi[] = "o1 !='$o1'";
					}else{
						$arrKondisi[] = " id_tahap='$idTahap'  or id_anggaran = '$id_anggaran' ";
						...
						while($row2s = sqlArray($getAllRekening)){
							...
							$cekRekening = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where k ='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
							if($cekRekening == 0){
								$concat = $k.".".$l.".".$m.".".$n.".".$o;
								$arrKondisi[] = "concat(k,'.',l,'.',m,'.',n,'.',o)  !='$concat'";
							}else{
								...
								while($row3s = sqlArray($getAllProgram)){
									...
									$cekProgram = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
									if($cekProgram == 0){
										$concat = $bk.".".$ck.".".$p;
										$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
									}else{
										...
										while($row4s = sqlArray($getAllKegiatan)){
											...
											$cekKegiatan = sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where bk ='$bk' and ck='$ck' and p='$p' and q='$q' and id_tahap ='$idTahap' $kondisiRekening $kondisiSKPD and (rincian_perhitungan !='' or rincian_perhitungan!='') "));
											if($cekKegiatan == 0){
												$concat = $bk.".".$ck.".".$p;
												$arrKondisi[] = "concat(bk,'.',ck,'.',p)  !='$concat'";
											}else{
				...
				$grabNonMapingRekening= sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$idTahap' and (LENGTH(k) > 1) and rincian_perhitungan=''");
				while($got = sqlArray($grabNonMapingRekening)){
					if(sqlNumRow(sqlQuery("select * from view_rka_ppkd_2_1 where id_tahap ='$idTahap' and k ='".$got['k']."' and l ='".$got['l']."' and m ='".$got['m']."' and n ='".$got['n']."' and o ='".$got['o']."' and rincian_perhitungan!='' $kondisiRekening $kondisiSKPD ")) != 0){
						$arrKondisi[] = "id_tahap = '$idTahap' or id_anggaran ='".$got['id_anggaran']."'";
					}
	...
	function Laporan($xls =FALSE){
		...
		while($daqry = sqlArray($aqry)){
			...
			if($k == '0' && $n =='0' ){
				...
				$getSumJumlahHarga = sqlArray(sqlQuery("select sum(jumlah_harga) from view_rka_ppkd_2_1 where  o1 ='$o1' $kondisiSKPD $kondisiFilter  "));
				...
				$getSumJumlahHarga = sqlArray(sqlQuery("select sum(jumlah_harga) from view_rka_ppkd_2_1 where  k = '$k' and l='$l' and m='$m' and n='$n' and o='$o'  $kondisiSKPD $kondisiFilter"));
...
if(empty($rkaPPKD21_v2->tahun)){
	...
	$getCurrenttahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD21_v2->idTahap'"));
	$rkaPPKD21_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaPPKD21_v2->idTahap'"));
