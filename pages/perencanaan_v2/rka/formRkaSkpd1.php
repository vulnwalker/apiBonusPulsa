...
class formRkaSkpd1_v2Obj  extends DaftarObj2{	
	...
	var $SHOW_CEK = TRUE;
	var $TblName = 'temp_rka_1_v2'; //bonus
	var $TblName_Hapus = 'temp_rka_1_v2';
	var $MaxFlush = 10;
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'finish':{		
			...
			}elseif(sqlNumRow(sqlQuery("select * from temp_rka_1_v2  where user ='$username' and delete_status = '0'")) == 0){
				$err = "Data kosong";
			}
			
			if(empty($err)){
				$execute = sqlQuery("select * from temp_rka_1_v2  where user ='$username' ");
				while($rows = sqlArray($execute)){
					...
					if(sqlNumRow(sqlQuery($queryCekForUpdate)) > 0){
						...
						if(sqlNumRow(sqlQuery("select * from view_rka_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k='$lamaK' and l='$lamaL' and m='$lamaM' and n='$lamaN' and o='$lamaO'  and (rincian_perhitungan !='' or f!='00') and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and id_tahap='$this->idTahap'")) == 0){
		...
		case 'SaveJob':{
			...
			$getMaxUrut = sqlArray(sqlQuery("select max(urut) from temp_rka_1_v2 where user ='$username'"));
		...
		case 'SaveEditJob':{
			...
			$getUrut = sqlArray(sqlQuery("select * from temp_rka_1_v2 where o1='$o1'"));
			$urut = $getUrut['urut'];
		...
		case 'cekNoUrut':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $username = $_COOKIE['coID'];
			 $cekRow = sqlNumRow(sqlQuery("select * from temp_rka_1_v2  where o1 = '$noPekerjaan'   and user ='$username' "));
			 if($cekRow == 0){
			 	$get = sqlArray(sqlQuery("select max(urut) from temp_rka_1_v2  where  user ='$username' and delete_status !='1' and o1 !='0' "));
			 	$urut = $get['max(urut)'] + 1;
			 }else{
			 	 $get = sqlArray(sqlQuery("select * from temp_rka_1_v2 where o1 = '$noPekerjaan'   and user ='$username' "));
				 $urut = $get['urut'];
			 }
		...
		case 'hapus2':{
	    	 $id = $_REQUEST['id'];
			 $get = sqlArray(sqlQuery("select * from temp_rka_1_v2 where id='$id' "));
			 $username = $_COOKIE['coID'];
			 $noPekerjaan = $get['o1'];
			 $noUrutPekerjaan = $get['urut'];
			 sqlQuery("update  temp_rka_1_v2 set delete_status = '1', o1 ='0' where id='$id'");
			 $execute = sqlQuery("select * from temp_rka_1_v2  where user='$username' and o1='$noPekerjaan' and delete_status = '0' order by o1, rincian_perhitungan");
			 $angkaUrut = 1;
			 while($rows = sqlArray($execute)){
				...
				$dataEditNoUrut = array('urut' => $noUrutPekerjaan,
			 						 	'o2' => $angkaUrut);
				$idTemp = $rows['id'];
				sqlQuery(VulnWalkerUpdate("temp_rka_1_v2",$dataEditNoUrut," id='$idTemp'"));
				$angkaUrut = $angkaUrut + 1;
				
				$content .= VulnWalkerUpdate("temp_rka_1_v2",$dataEditNoUrut," id='$idTemp'");
			 }
		break;
	    }
		...
		case 'saveEdit':{
	    	foreach ($_REQUEST as $key => $value) { 
			 ...
			 $query = VulnWalkerUpdate("temp_rka_1_v2",$data,"id='$id'");
			 sqlQuery($query);
			 $execute = sqlQuery("select * from temp_rka_1_v2  where user='$username' and o1='$noPekerjaan' and delete_status = '0' order by o1, rincian_perhitungan");
			 $angkaUrut = 1;
			 while($rows = sqlArray($execute)){
		...
		case 'edit':{
				...
				sqlQuery(VulnWalkerInsert('temp_alokasi_rka_1_v2',$dataAlokasi));
				/*if($satuan_total == ''){
					$statusAlokasi = 'false';
				}else{*/
					$statusAlokasi = 'true';
   ...
   function setPage_OtherScript(){
	...
	$Koloms[] = array('align="center" width="10"',$no );
		if($f != '00' || $rincian_perhitungan != ''){
			...
			$getTotal = sqlArray(sqlQuery("select sum(volume_rek) as volRek, sum(jumlah_harga) as jumlahHarga from temp_rka_1_v2 where o1 ='$o1' and user='$username'  and delete_status !='1'"));
	...
	$arrayJenisPerhitungan = array(
	 ...
	 $username = $_COOKIE['coID'];
	 $getAlokasi = sqlArray(sqlQuery("select * from temp_alokasi_rka_1_v2 where user='$username'"));
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrKondisi = array();		
		$username = $_COOKIE['coID'];
		$getAll = sqlQuery("select * from temp_rka_1_v2 where rincian_perhitungan ='' and user = '$username' and o1!='0'");
		while($rows = sqlArray($getAll)){
			foreach ($rows as $key => $value) { 
				 	 	$$key = $value; 
			}
			if(sqlNumRow(sqlQuery("select * from temp_rka_1_v2 where user ='$username' and rincian_perhitungan !='' and o1='$o1' and delete_status = '0' ")) == 0){
				sqlQuery("delete from temp_rka_1_v2 where id='$id'");				
			}else{
