...
class apbd4_v2Obj  extends DaftarObj2{	
	...
	$getJumlahBarang = sqlArray(sqlQuery("select * from tabel_anggaran where id_anggaran = '$apbd4_v2_idplh'"));
	...
	$getPaguYangTelahTerpakai = sqlArray(sqlQuery("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran'  and no_urut = '$this->nomorUrut' and id_anggaran!='$apbd4_v2_idplh' "));
	$sisaPaguIndikatif = $paguIndikatif - $getPaguYangTelahTerpakai['paguYangTerpakai'];
    
	 if(sqlNumRow(sqlQuery("select * from view_apbd4 where c1='0' and f = '00' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and id_tahap='$this->idTahap' ")) > 0){
...
/*		$codeAndNameUnit = "select e, concat(e, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and e!='00' and e1='000' ";
	...
	if($this->jenisForm == "KOREKSI" || $this->jenisForm == "PENYUSUNAN" || $this->jenisForm == "VALIDASI"){
		...
		$getPaguYangTerpakai =  sqlArray(sqlQuery("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and id_tahap = '$this->idTahap'  "));
		...
		$getPaguYangTerpakai =  sqlArray(sqlQuery("select sum(jumlah_harga) as paguYangTerpakai from view_apbd4 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and no_urut = '$this->urutTerakhir' and tahun ='$this->tahun' and jenis_anggaran = '$this->jenisAnggaran'  "));
		$sisaPagu = $getPaguIndikatif['jumlah'] - $getPaguYangTerpakai['paguYangTerpakai'];
		$sisaPagu =  number_format($sisaPagu ,2,',','.');
		$paguIndikatif = "<input type='text' value='$angkaPaguIndikatif' readonly> &nbsp &nbsp &nbsp SISA PAGU  :  <input type='text' value='$sisaPagu' readonly>";
	...
	function getDaftarOpsi($Mode=1){
		...
		if(!isset($cmbUrusan) ){
			...
			while($rows = sqlArray($getAllFromRka221)){
				...
				if(sqlArray(sqlQuery("select * from view_apbd4 where c1 = '$c1' and c='00' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => '00',
				...
				if(sqlArray(sqlQuery("select * from view_apbd4 where c1 = '$c1' and c='$c' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_apbd4 where c1 = '$c1' and c='$c' and d='$d' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_apbd4 where  c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='0' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_apbd4 where c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
		...
		/*$getAllProgram = sqlQuery("select * from view_apbd4 where q != '0'");
		$arrayProgram = array();
		while($rows = sqlArray($getAllProgram)){
			foreach ($rows as $key => $value) { 
				  $$key = $value; 
			}
			if(sqlNumRow(sqlQuery("select * from view_apbd4 where   (rincian_perhitungan !='' or f ='00' ) and bk='$bk' and ck='$ck' and p='$p' and q='$q'  $kondisiFilter $kondisiSKPD")) == 0){
				$arrKondisi[] = "id_anggaran !='$id_anggaran'";	
				if(sqlNumRow(sqlQuery("select * from view_apbd4 where   (rincian_perhitungan !='' or f ='00' ) and bk='$bk' and ck='$ck' and p='$p'   $kondisiFilter $kondisiSKPD")) == 0){
					$concat = $bk.".".$ck.".".$p;
					$arrKondisi[] = "concat(bk,'.',ck,'.',p) !='$concat'";
				}
			}else{
					$getIDProgram =  sqlArray(sqlQuery("select * from view_apbd4 where bk='$bk' and ck='$ck' and p='$p' and  q = '0'"));
...
if(empty($apbd4_v2->tahun)){
	...
	$get1 = sqlArray(sqlQuery("select max(id_anggaran)  from view_apbd4 "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = sqlArray(sqlQuery("select * from view_apbd4 where id_anggaran = '$maxAnggaran'"));
	...
	$getCurrenttahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$apbd4_v2->idTahap'"));
	$apbd4_v2->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$apbd4_v2->idTahap'"));
