...
class r_apbd5Obj  extends DaftarObj2{	
	...
	$getJumlahBarang = sqlArray(sqlQuery("select * from tabel_anggaran where id_anggaran = '$r_apbd5_idplh'"));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'sesuai':{
			...
			$getr_apbd5nya = sqlArray(sqlQuery($queryRows));
		...
		case 'koreksi':{
			...
			foreach ($getr_apbd5nya as $key => $value) { 
			 ...
			 if($err == ""){
				 	if(sqlNumRow(sqlQuery("select * from view_r_apbd5_1 where o1='0' and rincian_perhitungan ='' and c1='0' and k='$k' and l='$l' and m='$m' and n='$n' and o='$o' and id_tahap='$this->idTahap'  ")) > 0){
					 ...
					 if(sqlNumRow(sqlQuery("select * from view_r_apbd5_1 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and k = '$k' and l ='$l' and m='$m' and n='$n' and o='$o'  and o1='$o1' and f1='0' and rincian_perhitungan ='' and id_tahap='$this->idTahap' ")) > 0){
		...
		case 'editTab':{
			...
			$getAll = sqlQuery("select * from view_r_apbd5_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit' and bk='$bk' and ck='$ck' and p='$hiddenP' and q='$q' order by o1, rincian_perhitungan");
			sqlQuery("delete from temp_rka_221 where user='$username'");
...
/*		$codeAndNameUnit = "select e, concat(e, '. ', nm_skpd) from ref_skpd where c1='$selectedC1' and c='$selectedC' and d='$selectedD' and e!='00' and e1='000' ";
	...
	if($this->jenisForm == "KOREKSI" || $this->jenisForm == "PENYUSUNAN" || $this->jenisForm == "VALIDASI"){
		...
		$getPaguYangTerpakai =  sqlArray(sqlQuery("select sum(jumlah_harga) as paguYangTerpakai from view_r_apbd5_1 where c1= '$selectedC1' and c='$selectedC' and d='$selectedD' and e='$selectedE' and e1='$selectedE1' and bk='$selectedBK' and ck='$selectedCK' and p='$selectedP' and q='$selectedQ' and id_tahap = '$this->idTahap'  "));
	...
	function getDaftarOpsi($Mode=1){
		...
		foreach ($_REQUEST as $key => $value) { 
			...
			while($rows = sqlArray($getAllFromRka221)){
				...
				if(sqlArray(sqlQuery("select * from view_r_apbd5 where c1 = '$c1' and c='00' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => '00',
				...
				if(sqlArray(sqlQuery("select * from view_r_apbd5 where c1 = '$c1' and c='$c' and d='00' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_r_apbd5 where c1 = '$c1' and c='$c' and d='$d' and e='00' and e1='000' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_r_apbd5 where  c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='0' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
				...
				if(sqlArray(sqlQuery("select * from view_r_apbd5 where c1='$c1' and c='$c' and d='$d' and e='00' and e1='000' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
		...
		/*$getAllProgram = sqlQuery("select * from view_r_apbd5 where q != '0'");
		$arrayProgram = array();
		while($rows = sqlArray($getAllProgram)){
			foreach ($rows as $key => $value) { 
				  $$key = $value; 
			}
			if(sqlNumRow(sqlQuery("select * from view_r_apbd5_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p' and q='$q'  $kondisiFilter $kondisiSKPD")) == 0){
				$arrKondisi[] = "id_anggaran !='$id_anggaran'";	
				if(sqlNumRow(sqlQuery("select * from view_r_apbd5_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p'   $kondisiFilter $kondisiSKPD")) == 0){
					$concat = $bk.".".$ck.".".$p;
					$arrKondisi[] = "concat(bk,'.',ck,'.',p) !='$concat'";
				}
			}else{
					$getIDProgram =  sqlArray(sqlQuery("select * from view_r_apbd5 where bk='$bk' and ck='$ck' and p='$p' and  q = '0'"));
...
if(empty($r_apbd5->tahun)){
	...
	$get1 = sqlArray(sqlQuery("select max(id_anggaran)  from view_r_apbd5_1 "));
	$maxAnggaran = $get1['max(id_anggaran)'];
	$get2 = sqlArray(sqlQuery("select * from view_r_apbd5_1 where id_anggaran = '$maxAnggaran'"));
	...
	$getCurrenttahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$r_apbd5->idTahap'"));
	$r_apbd5->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$r_apbd5->idTahap'"));
