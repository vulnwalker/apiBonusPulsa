...
class ref_jadwalObj  extends DaftarObj2{	
	...
	if($penyocokanTanggal > 0){
		$err ="TANGGAL SALAH";
				...
				if($err==''){
					...
					sqlQuery($query);
					$getIDTahap = sqlArray(sqlQuery("select max(id_tahap) as ID_TAHAP from ref_jadwal"));
					...
					$dataDefault = array("tahun" => $tahun,
					 ...
					 $nomor_urut = 1;
								$execute = sqlQuery("select * from ref_jadwal where status_penyusunan ='AKTIF' and anggaran='$anggaran' and  tahun = '$tahun' ORDER BY id_tahap");
								while($rows = sqlArray($execute)){
									$dataUpdateAll = array("no_urut" => $nomor_urut);
									$currentIdTahap = $rows['id_tahap'];
									sqlQuery(VulnWalkerUpdate('ref_jadwal',$dataUpdateAll," id_tahap = '$currentIdTahap'") );
	...
	function setForm($dt){	
	 ...
	 $this->form_height = 400;
	  if ($this->form_fmST==0) {
		...
		$status = "PASIF";
		$getMaxNomorAku = sqlArray(sqlQuery("select max(id_tahap) max from ref_jadwal"));
		$maxNomorAku = $getMaxNomorAku['max'];
		$getNomorAku = sqlArray(sqlQuery("select * from ref_jadwal where id_tahap = '$maxNomorAku'"));
   ...
   $arrayAnggaran = array(	array('MURNI','MURNI'),
	...
	$maxNoUrut = $nomor_aku ;
	$getDataMax = sqlArray(sqlQuery("select * from ref_jadwal where no_urut = '$maxNoUrut' " ));
...
$getDataMaxSetelah = sqlArray(sqlQuery("select * from ref_jadwal where no_urut = '$maxNoUrutSesudahnya' " ));
