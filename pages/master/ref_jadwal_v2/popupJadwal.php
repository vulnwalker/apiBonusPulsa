	  ...
	  switch($tipe){
		...
		case 'windowshow':{
	   	...
	   	case 'getdata':{
			...
			if(empty($tahun)){
				...
				$getData = sqlQuery("select * from ref_jadwal_v2 where status_aktif = 'AKTIF'  order by id");
				$nomorUrut = 1 ;
	   ...
	   case 'save':{
		...
		break;
	   	...
	   	case 'newRow':{
			$getMaxNoUrut = sqlArray(sqlQuery("select max(no_urut) from ref_jadwal_v2"));
			$no_urut = $getMaxNoUrut['max(no_urut)'];
			sqlQuery("insert into ref_jadwal_v2 (nama_tahap) values ('')");
		break;
	   }
	   	case 'cekDataAda':{
			foreach ($_REQUEST as $key => $value) { 
	   ...
	   case 'modulChanged':{
		...
		break;
	   	...
	   	case 'remove':{
			$id =  $_REQUEST['id'];
			sqlQuery("delete from ref_jadwal_v2 where id='$id'");
		break;
	   }
	    	case 'cancel':{
			$id =  $_REQUEST['id'];
			sqlQuery("delete from ref_jadwal_v2 where nama_tahap =''");
		break;
	   }
