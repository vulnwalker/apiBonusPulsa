	...
	function set_selector_other($tipe){
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		$get = sqlArray( sqlQuery("select * from ref_barang where f1=$f1 and f2=$f2 and  f=$f and g=$g and h=$h and i=$i and j=$j"));
		$kode_barang=$get['f1'].'.'.$get['f2'].'.'.$get['f'].'.'.$get['g'].'.'.$get['h'].'.'.$get['i'].'.'.$get['j'];
		 ...
		 $getKebutuhanOptimal = sqlArray(sqlQuery("select sum(jml_barang) as kebutuhanOptimal from buku_induk where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and (kondisi = '1' or kondisi = '2' or kondisi = '3') "));	
...
/*			'' => array( 
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $getMinID = sqlArray(sqlQuery("select min(Id) as min from view_barang_rkbmd_pemeliharaan where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat'"));
