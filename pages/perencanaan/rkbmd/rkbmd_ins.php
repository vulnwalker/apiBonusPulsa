...
class rkbmd_insObj  extends DaftarObj2{	
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'subDelete':{				
			...
			sqlQuery("delete from temp_rkbmd_pengadaan where id='$id'");	
			$getRow = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan where user = '$username'"));	
			if($getRow > 0){
				$content = "ada";
			}else{
				$content = "";
		...
		case 'finish':{				
			$username = $_COOKIE['coID'];
			$execute = sqlQuery("select * from temp_rkbmd_pengadaan where user='$username'");	
			...
			$cekKegiatan = sqlNumRow(sqlQuery("select * from view_rkbmd where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and ck = '$ck' and p = '$p' and q= '$q' and  f1='0' and id_tahap='$this->idTahap' and uraian_pemeliharaan = 'RKBMD PENGADAAN'"));												
			if($cekKegiatan < 1){
				$data = array('jenis_anggaran' => $this->jenisAnggaran,
							  'tahun' => $this->tahun,
							  'c1' => $c1,
			...
			$execute2 = sqlQuery("select * from temp_rkbmd_pengadaan where user='$username'");
		...
		case 'Simpan':{
			...
			$username = $_COOKIE['coID'];
			$cekAda = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan where f1='$f1' and f2='$f2' and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' and user='$username'"));
			if($cekAda == 1){
   ...
   function setPage_OtherScript(){
	...
	$cekGordon = sqlNumRow(sqlQuery("select * from temp_rkbmd_pengadaan where user = '$username'"));
	if($cekGordon > 0){
		$cokot = sqlArray(sqlQuery("select * from temp_rkbmd_pengadaan where user = '$username'"));
		$selectedBK = $cokot['bk'];
		$selectedCK = $cokot['ck'];
		$selectedP = $cokot['p'];
		$selecteQ = $cokot['q'];
