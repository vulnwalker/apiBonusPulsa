...
class rkaSKPD22Obj  extends DaftarObj2{	
	...
	$getJumlahBarang = sqlArray(sqlQuery("select * from tabel_anggaran where id_anggaran = '$rkaSKPD22_idplh'"));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'sesuai':{
			...
			$getrkaSKPD22nya = sqlArray(sqlQuery($queryRows));
		...
		case 'editTab':{
			...
			$getAll = sqlQuery("select * from view_rka_2_2_1 where concat(k,'.',l,'.',m,'.',n,'.',o) ='$kodeRek' and id_tahap='$this->idTahap' and rincian_perhitungan !=''   and c1='$cmbUrusan' and c='$cmbBidang' and d='$cmbSKPD' and e='$cmbUnit' and e1='$cmbSubUnit' and bk='$bk' and ck='$ck' and p='$hiddenP' and q='$q' order by o1, rincian_perhitungan");
			sqlQuery("delete from temp_rka_221 where user='$username'");
	...
	function getDaftarOpsi($Mode=1){
		...
		foreach ($_REQUEST as $key => $value) { 
			...
			while($rows = sqlArray($getAllFromRka221)){
				...
				if(sqlArray(sqlQuery("select * from view_rka_2_2 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' ")) > 0){
				}else{
					$data = array(
									'c1' => $c1,
									'c' => $c,
		...
		while($rows = sqlArray($getAllProgram)){
			...
			if(sqlNumRow(sqlQuery("select * from view_rka_2_2_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p' and q='$q'  $kondisiFilter $kondisiSKPD")) == 0){
				$arrKondisi[] = "id_anggaran !='$id_anggaran'";	
				if(sqlNumRow(sqlQuery("select * from view_rka_2_2_1 where  o1 !='0' and (rincian_perhitungan !='' or f1 !='0' ) and bk='$bk' and ck='$ck' and p='$p'   $kondisiFilter $kondisiSKPD")) == 0){
					$concat = $bk.".".$ck.".".$p;
...
if(empty($rkaSKPD22->tahun)){
	...
	$getCurrenttahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD22->idTahap'"));
	$rkaSKPD22->currentTahap = $getCurrenttahap['nama_tahap'];
	
	$namaTahap = sqlArray(sqlQuery("select * from ref_tahap_anggaran where id_tahap = '$rkaSKPD22->idTahap'"));
