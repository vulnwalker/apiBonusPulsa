...
class rkbmd_pemeliharaan_v3Obj  extends DaftarObj2{	
	...
	var $SHOW_CEK = TRUE;
	var $TblName = 'rkbmd_pemeliharaan_v3'; //bonus
	...
	function setTitle(){
	    ...
	    $getTahun = sqlArray(sqlQuery("select * from tabel_anggaran_v3 where id_anggaran = '$id'"));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'CekAda':{
			...
			$getAll = sqlQuery("select * from view_rkbmd_v3 where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and id_jenis_pemeliharaan !='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and status_validasi !='1'");
			...
			$content = array("status" => $tergantung, "query" => "select * from view_rkbmd_v3 where tahun ='$this->tahun' and jenis_anggaran ='$this->jenisAnggaran' and jenis_form_modul ='PENYUSUNAN' and id_tahap='$this->idTahap' and j !='000' and id_jenis_pemeliharaan ='0' and c1='$c1' and c='$c' and d='$d' and e='$e' and e1 ='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q'");
		...
		case 'finish':{		
			$username = $_COOKIE['coID'];		
			$execute = sqlQuery("select * from rkbmd_pemeliharaan_v3 where user='$username'");
			$get = sqlArray($execute);
			foreach ($get as $key => $value) { 
				  $$key = $value; 
			}
			if(sqlNumRow(sqlQuery("select * from rkbmd_pemeliharaan_v3 where user='$username'")) == 0){
				$err = "Data Kosong";
			    ...
			    sqlQuery("delete from tabel_anggaran_v3 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk='$bk' and ck='$ck' and p='$p' and q='$q' and tahun='$this->tahun' and id_tahap ='$this->idTahap' and ((id_jenis_pemeliharaan != '0' and f !='00') or uraian_pemeliharaan = 'RKBMD PEMELIHARAAN') and status_validasi !='1' ");
				...
				$cekKegiatan = sqlNumRow(sqlQuery("select * from view_rkbmd_v3 where c1='$c1' and c='$c' and d='$d' and e='$e' and e1='$e1' and bk = '$bk' and ck = '$ck' and p = '$p' and q= '$q' and  f='00' and id_tahap='$this->idTahap' and uraian_pemeliharaan = 'RKBMD PEMELIHARAAN'"));												
				if($cekKegiatan < 1){
					$data = array('jenis_anggaran' => $this->jenisAnggaran,
								  'tahun' => $this->tahun,
								  'c1' => $c1,
				...
				$execute2 = sqlQuery("select * from rkbmd_pemeliharaan_v3 where user='$username'");
				while($rows = sqlArray($execute2)){
					...
					$data = array('jenis_anggaran' => $this->jenisAnggaran,
						...
						sqlQuery(VulnWalkerInsert("tabel_anggaran_v3",$data));
						$content = VulnWalkerInsert("tabel_anggaran_v3",$data);
						sqlQuery("delete from rkbmd_pemeliharaan_v3 where id = '$id'");
				}	
			}
		...
		case 'newRowPemeliharaan':{
			...
			$query = (VulnWalkerInsert('temp_rkbmd_pemeliharaan_v3',$data));
			sqlQuery($query);
			$username = $_COOKIE['coID'];
			$getIdAkhir = sqlArray(sqlQuery("select max(id) as idAkhir from temp_rkbmd_pemeliharaan_v3 where user = '$username'"));
		...
		case 'CekKosong':{				
			...
			$query = "select * from view_rkbmd_v3 where c1 = '$c1' and c ='$c' and d ='$d' and p !='00' and q != '00' and tahun = '$tahunAnggaran'";
			...
			$query = "select * from view_rkbmd_v3 where c1 = '$c1' and c ='$c' and d !='00' and p !='00' and q != '00' and tahun = '$tahunAnggaran'";
			$ada = sqlNumRow(sqlQuery($query));
			if($ada > 0){
			}else{
			...
			$query = "select * from view_rkbmd_v3 where  c1 = '$c1' and c !='00' and d !='00' and p !='00' and q != '00' and tahun = '$tahunAnggaran'";
			$ada = sqlNumRow(sqlQuery($query));
			if($ada > 0){
			}else{
		...
		case 'Simpan':{
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];			
			$cekSame = sqlNumRow(sqlQuery("select * from rkbmd_pemeliharaan_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'"));
	...
	$cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');  
		...
		case 'subSubDelete':{				
			$id = $_REQUEST['id'];
			sqlQuery("delete from temp_rkbmd_pemeliharaan_v3 where id='$id'");				
		break;
		}
		case 'subHapus':{				
			$id = $_REQUEST['id'];
			$get = sqlArray(sqlQuery("select * from rkbmd_pemeliharaan_v3 where id ='$id'"));
			foreach ($get as $key => $value) { 
			  $$key = $value; 
			}
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$username = $_COOKIE['coID'];	 
			$execute = sqlQuery("select * from rkbmd_pemeliharaan_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'  and user = '$username'");	
		...
		case 'subSubSave':{				
			...
			$data = array(
				...
				$query=VulnWalkerUpdate('temp_rkbmd_pemeliharaan_v3',$data,"id = '$id'");
				
				$concat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$bk.".".$ck.".".$dk.".".$p.".".$q.".".$f.".".$g.".".$h.".".$i.".".$j;
				$cekSame = sqlNumRow(sqlQuery("select * from temp_rkbmd_pemeliharaan_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',dk,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and id_jenis_pemeliharaan ='$idJenisPemeliharaan' and id !='$id'"));
				...
				$content = "select * from temp_rkbmd_pemeliharaan_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',dk,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and id_jenis_pemeliharaan ='$idJenisPemeliharaan' and id !='$id'";
		...
		case 'subSubCancel':{	
		    $username = $_COOKIE['coID'];			
			sqlQuery("delete from temp_rkbmd_pemeliharaan_v3 where user = '$username' and (id_jenis_pemeliharaan = '' OR id_jenis_pemeliharaan = '0' )");				
		break;
		}
		case 'clear':{	
			 $username = $_COOKIE['coID'];
		...
		case 'subCancel':{	
			...
			$username = $_COOKIE['coID'];	
			sqlQuery("delete from temp_rkbmd_pemeliharaan_v3 where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'");				
		break;
		}
		...
		case 'moveBack':{	
			...
			$concat = $kodeBarang;
			$username =$_COOKIE['coID'];
			$execute = sqlQuery("select * from temp_rkbmd_pemeliharaan_v3 where user = '$username' and (id_jenis_pemeliharaan != '0' or  id_jenis_pemeliharaan !='')");
			if(sqlNumRow($execute) == 0){
				$err = "Data kosong !";
			}else{	
			sqlQuery("delete from rkbmd_pemeliharaan_v3 where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'");
					while($rows = sqlArray($execute)){
					 foreach ($rows as $key => $value) { 
				  		$$key = $value; 
					 }
					$data = array(
					 ...
					 $query = VulnWalkerInsert('rkbmd_pemeliharaan_v3',$data);
					 sqlQuery($query);
					 sqlQuery("delete from temp_rkbmd_pemeliharaan_v3 where id = '$id'");
					 $codeAndNameKegiatan = "select q, concat(q,'. ', nama) from ref_program where bk = '$bk' and ck = '$ck'  and p = '$p' and q ='$q' ";
					 $cmbKegiatan = cmbQuery('q', $q, $codeAndNameKegiatan,' disabled','-- KEGIATAN --');  
		...
		case 'moveList':{	
			...
			$concat = $f.".".$g.".".$h.".".$i.".".$j;
			$execute = sqlQuery("select * from rkbmd_pemeliharaan_v3 where user = '$username' and concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'");
			sqlQuery("delete from temp_rkbmd_pemeliharaan_v3 where user = '$username'");
			while($rows = sqlArray($execute)){
				foreach ($rows as $key => $value) { 
			  		$$key = $value; 
			...
			$getNamaBarang = sqlArray(sqlQuery("select * from ref_barang_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat'"));
			$namaBarang = $getNamaBarang['nm_barang'];
			$satuan = $satuan;
			$concat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$f.".".$g.".".$h.".".$i.".".$j;
			$getJumlah = sqlArray(sqlQuery("select sum(jml_barang) as jumlah from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and (kondisi = '1' or  kondisi = '2' or  kondisi = '3')"));
			$jumlah = $getJumlah['jumlah'];
			$getBaik = $getJumlah = sqlArray(sqlQuery("select sum(jml_barang) as jumlah from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '1' "));
			$baik = $getBaik['jumlah'];
			$getKurangBaik = $getJumlah = sqlArray(sqlQuery("select sum(jml_barang) as jumlah from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '2' "));
			$kurangBaik = $getKurangBaik['jumlah'];
			$getRusakBerat = $getJumlah = sqlArray(sqlQuery("select sum(jml_barang) as jumlah from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',f,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '3' "));
		...
		case 'subSimpan':{	
			...
			$username = $_COOKIE['coID'];
			$execute = sqlQuery("select * from temp_rkbmd_pemeliharaan_v3 where user = '$username' and id_jenis_pemeliharaan != '0' and  id_jenis_pemeliharaan !=''");
			if(sqlNumRow($execute) == 0){
				$err = "Data kosong !";
			}else{
					while($rows = sqlArray($execute)){
		...
		case 'subShowEdit':{
		   ...
		   $getBarang = sqlArray( sqlQuery("select * from ref_barang_v3 where   f=$f and g=$g and h=$h and i=$i and j=$j"));
   ...
   function setPage_OtherScript(){
	...
	$Koloms = array();
	$concat2 = $f.".".$g.".".$h.".".$i.".".$j;
	$getNamaBarang = sqlArray(sqlQuery("select * from ref_barang_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2'"));
	$concat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$f.".".$g.".".$h.".".$i.".".$j ; 
	...
	$getKondisiKurangBaik = sqlArray(sqlQuery("select sum(jml_barang) as kurangBaik from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '2'" ));		
	$kurangBaik = $getKondisiKurangBaik['kurangBaik'];
	$getKondisiRusakBerat = sqlArray(sqlQuery("select sum(jml_barang) as rusakBerat from buku_induk_v3 where concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',g,'.',h,'.',i,'.',j) = '$concat' and status_barang = '1' and kondisi = '3'" ));		
	...
	$hitungData = sqlNumRow(sqlQuery("select * from rkbmd_pemeliharaan_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
	if($hitungData > 1){
		$getMin = sqlArray(sqlQuery("select min(concat(c1,'.',c,'.',d,'.',e,'.',e1,'.',bk,'.',ck,'.',p,'.',q,'.',f,'.',g,'.',h,'.',i,'.',j,'.',id_jenis_pemeliharaan)) as min from rkbmd_pemeliharaan_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user = '$username'"));
		$min = $getMin['min'];
		$realConcat = $c1.".".$c.".".$d.".".$e.".".$e1.".".$bk.".".$ck.".".$p.".".$q.".".$f.".".$g.".".$h.".".$i.".".$j.".".$id_jenis_pemeliharaan; 
		if($realConcat == $min){
			$getTotal = sqlArray(sqlQuery("select sum(volume_barang) as total from rkbmd_pemeliharaan_v3 where concat(f,'.',g,'.',h,'.',i,'.',j) = '$concat2' and user='$username'"));
	...
	function tabelPemeliharaan(){
		...
		$qry = "select * from temp_rkbmd_pemeliharaan_v3 where id_jenis_pemeliharaan != '' and user = '$username'";$cek.=$qry;
		$aqry = sqlQuery($qry);
		$no=1;
	...
	$Syntax_ambil_yang_udah_ada_plafon = "select * from tabel_anggaran_v3 where id_anggaran='$ID_RENJA'";
	...
	$username = $_COOKIE['coID'];
	$matiButton = "";
	$syntax = sqlQuery("select * from rkbmd_pemeliharaan_v3 where user = '$username'");
...
$arrayResult = tahapPerencanaanV3($rkbmd_pemeliharaan_v3->modul);
