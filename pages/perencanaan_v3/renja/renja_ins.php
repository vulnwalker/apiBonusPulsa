	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){	
		...
		case 'CekKosong':{				
			...
			$query = "select * from view_renja_v3 where c1 = '$c1' and c ='$c' and d ='$d' and p !='00' and q != '00' and tahun = '$this->tahun'";
			$ada = sqlNumRow(sqlQuery($query));
			if($ada > 0){
			}else{
				sqlQuery("delete from renja where c1 = '$c1' and c ='$c' and d ='$d' and tahun = '$this->tahun' ");
			}
			
			
			$query = "select * from view_renja_v3 where c1 = '$c1' and c ='$c' and d !='00' and p !='00' and q != '00' and tahun = '$this->tahun'";
			$ada = sqlNumRow(sqlQuery($query));
			if($ada > 0){
			}else{
				sqlQuery("delete from renja where c1 = '$c1' and c ='$c' and d ='00' and tahun = '$this->tahun'");
				
			}
			
			$query = "select * from view_renja_v3 where  c1 = '$c1' and c !='00' and d !='00' and p !='00' and q != '00' and tahun = '$this->tahun'";
			$ada = sqlNumRow(sqlQuery($query));
			if($ada > 0){
			}else{
				sqlQuery("delete from renja where c1 = '$c1' and c ='00' and d ='00' and tahun = '$this->tahun' ");
...
/*			elseif(empty($paguIndikatif)){
		...
		if($this->jenisForm != "PENYUSUNAN"){
			...
			if($err == ''){
					...
					if($cekSKPD > 0 ){
						...
						$getTotalPagu = sqlArray(sqlQuery("select sum(jumlah) as paguTotal from view_renja_v3 where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and c1= '$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and id_tahap = '$this->idTahap'"));
						$totaPagu = $getTotalPagu['paguTotal'];
						
						$nomorUrutSebelumnya = $this->nomorUrut - 1;
						$getPlafon  =  sqlArray(sqlQuery("select plafon from view_plafon_v3 where tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and c1= '$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and no_urut = '$nomorUrutSebelumnya'"));
						...
						if( $angkaSekarang > $sisanya){
							$err = "Pagu tidak dapat melebihi Plafon !!! ";
								...
								$cekKegiatan = "select * from tabel_anggaran_v3  where jenis_anggaran = '$this->jenisAnggaran' and tahun = '$this->tahun' and c1= '$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and e = '$cmbUnitForm' and e1 = '$cmbSubUnitForm' and bk ='$bk' and ck ='$ck' and p = '$p' and q='$cmbKegiatan' and id_tahap = '$this->idTahap'";	
								if(sqlNumRow(sqlQuery($cekKegiatan)) == 0){
								$data = array(
												'jenis_anggaran' => $this->jenisAnggaran,
												'tahun' => $this->tahun,
												'c1' => $cmbUrusanForm,
								 ...
								 $queryK = VulnWalkerInsert("tabel_anggaran_v3",$data);
								 sqlQuery($queryK);
								 $getIDAnggaran = sqlArray(sqlQuery("select max(id_anggaran) as maxIDAnggaran from tabel_anggaran_v3  " ));
								 ...
								 $queryDetail = VulnWalkerInsert('detail_renja_v3',$dataDetailRenja);
								 sqlQuery($queryDetail);
		...
		$getSKPD = sqlArray(sqlQuery("select * from tabel_anggaran_v3 where id_anggaran = '$ID_RENJA'"));
		...
		$getTotalPagu = sqlArray(sqlQuery("select sum(jumlah) as paguTotal from view_renja_v3 where tahun = '$sTahun' and jenis_anggaran = '$sJenisAnggaran' and c1= '$cmbUrusanForm' and c='$cmbBidangForm' and d='$cmbSKPDForm' and id_tahap = '$this->idTahap'"));
			...
			if($err == ""){
				$cekSKPD =  sqlNumRow(sqlQuery("select * from tabel_anggaran_v3 where c1 = '$cmbUrusanForm' and c='$cmbBidangForm' and d = '$cmbSKPDForm' and e='00' and e1='000' and bk='0' and ck='0' and p = '0' and q='0' and p = '00' and q='00'  and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and id_tahap = '$this->idTahap' "));
					...
					$data = array( "jenis_kegiatan" => $jenisKegiatan,
							...
							$query = VulnWalkerUpdate('detail_renja_v3',$data,"id_anggaran='$ID_RENJA'");			
							sqlQuery($query);
				...
				sqlQuery(VulnWalkerUpdate("tabel_anggaran_v3",$dataAnggaran," id_anggaran = '$ID_RENJA' "));	
			}		
		}
   ...
   function setPage_OtherScript(){
	...
	$getPengurangan = sqlArray(sqlQuery("select sum(jumlah) as jumlah from view_renja_v3 where q != 0 and tahun = '$this->tahun' and jenis_anggaran = '$this->jenisAnggaran' and c1 ='$c1' and c='$c' and d='$d'"));
	...
	if($ID_RENJA != '' && $ID_RENJA > 0){
	    $getRenja = sqlArray(sqlQuery("select * from tabel_anggaran_v3 where id_anggaran = '$ID_RENJA'"));
		...
		$getNamaRefUrusan = sqlArray(sqlQuery("select * from ref_urusan where bk = '$bk' and ck ='0' and dk='0'"));
		$urusanProgram = $getNamaRefUrusan['nm_urusan'];
		$getNamaRefBidang = sqlArray(sqlQuery("select * from ref_urusan where bk = '$bk' and ck ='$ck' and dk='0'"));
		$bidangProgram = $getNamaRefBidang['nm_urusan'];
		$getNamaProgram = sqlArray(sqlQuery("select * from ref_program where bk = '$selectedBK' and ck ='$selectedCK' and p='$selectedProgram' and q='0'"));
		$program = $getNamaProgram ['nama'];
		$getDetail = sqlArray(sqlQuery("select * from detail_renja_v3 where id_anggaran = '$ID_RENJA'"));
...
$waktuPelaksanaan = $mulai."&nbsp&nbsp s/d &nbsp&nbsp ".$selesai." &nbsp&nbsp ".$tahunJamak;
	...
	function rincianpenerimaanDET(){
		...
		$ID_RENJA = $_REQUEST['ID_RENJA'];
		$getKeterangan = sqlArray(sqlQuery("select * from detail_renja_v3 where id_anggaran= '$ID_RENJA'"));
...
$arrayResult = tahapPerencanaanV3($renja_ins_v3->modul);
